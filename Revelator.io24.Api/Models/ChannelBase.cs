...
namespace Presonus.StudioLive32.Api.Models
{
	...
	public class ChannelBase : DeviceRoutingBase, INotifyPropertyChanged
	{
		...
		private PeakValueTracker channelMeter;
		protected PeakValueTracker gateMeter;
		private PeakValueTracker compMeter;
		private PeakValueTracker limiterMeter;
		...
		{
			this.device = device;
			channelMeter = new PeakValueTracker(1024);
			gateMeter = new PeakValueTracker(1024);
			limiterMeter = new PeakValueTracker(1024);
			compMeter = new PeakValueTracker(1024);
		}
		...
		public float meter
		{
			...
			{
				var val = GetValue();
				channelMeter.AddSample(val);
				return (float)channelMeter.GetPeakValue();
		...
		public float comp_reduction
		{
			...
			{
				var val = GetValue();
				compMeter.AddSample(val);
				return (float)compMeter.GetPeakValue();
			}
		}
		...
		public float limiter_reduction
		{
			...
			{
				var val = GetValue();
				limiterMeter.AddSample(val);
				return (float)limiterMeter.GetPeakValue();
			}
		}
		#endregion
	}

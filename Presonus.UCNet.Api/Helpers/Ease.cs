...
namespace Presonus.UCNet.Api.Helpers
{
	public static class Ease
	{
		...
		public static readonly Easer LogIn = (float t) => { return (float)Math.Log10(9 * t + 1); };
		public static readonly Easer LogOut = Invert(LogIn);
		public static readonly Easer LogInOut = Follow(LogIn, LogOut);

		public static readonly Easer Log = (float value)=> (float)Math.Pow(10, value) - 1;


		public static readonly Easer SkewIn = (float t) => { return (float)Math.Pow(t, 4.285f); };
		public static Easer CreatePowerInEasing(float exponent)
		{
			return (float t) => { return (float)Math.Pow(t, exponent); };
		}

		public static Easer CreatePowerOutEasing(float exponent)
		{
			Easer powerInEaser = CreatePowerInEasing(exponent);
			return Invert(powerInEaser);
		}

		public static Easer CreatePowerInOutEasing(float exponent)
		{
			Easer powerInEaser = CreatePowerInEasing(exponent);
			Easer powerOutEaser = Invert(powerInEaser);
			return Follow(powerInEaser, powerOutEaser);
		}

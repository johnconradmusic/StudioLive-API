...
using MixingStation.Api.Extensions;
using MixingStation.Api.Helpers;
using MixingStation.Api.Messages;
using MixingStation.Api.Messages.Readers;
...
namespace MixingStation.Api.Services
{
    public class CommunicationService : IDisposable
    {
        ...
        static void PrintByteArray(byte[] byteArray)
        {
            ...
            {
                byte[] lineBytes = new byte[Math.Min(bytesPerLine, byteArray.Length - i)];
                Array.Copy(byteArray, i, lineBytes, 0, lineBytes.Length);

                string hexLine = BitConverter.ToString(lineBytes);
                string[] hexValues = hexLine.Split('-');

                string printableLine = string.Join(" ", hexValues);

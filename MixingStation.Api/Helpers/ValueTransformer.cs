...
using static MixingStation.Api.Helpers.Ease;
...
namespace MixingStation.Api.Helpers
{
    ...
    public class ValueTransformer
    {
        ...
        public static double MapToPseudoExponential(double x, double min, double max, double mid)
        {
            if (x < 0 || x > 1)
            {
                throw new ArgumentOutOfRangeException("x", "x must be between 0 and 1");
            }

            // Calculate the power that makes the midpoint correct.
            double power = Math.Log(0.5) / Math.Log((mid - min) / (max - min));

            // Apply the power function and scale to the desired range.
            return min + Math.Pow(x, power) * (max - min);
        }
        ...
        public static string Transform(float input_value, float min_value, float max_value, float mid_value, CurveFormula curve_formula, Units units)
        {
